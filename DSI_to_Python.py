...
class TCPParser: # The script contains one main class which handles DSI-Streamer data packet parsing.
	...
	def parse_data(self):
		...
		while not self.done:
			...
			# if data header @ABCD is present, start looking for packets
			if self.data_log.find(b'@ABCD',0,len(self.data_log)) != -1:										
				...
				# For each packet, append the signal data and timestamps to their respective logs.
	...
	def example_plot(self):
		...
		while True: # runtime < duration/refresh_rate:
			...
			plt.xlabel('Timestamp (runtime={})'.format(runtime))
